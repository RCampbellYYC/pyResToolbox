...
def gas_ug(p: npt.ArrayLike, sg: float, degf: float, zmethod: z_method=z_method.DAK, cmethod: c_method=c_method.PMC, n2: float = 0, co2: float = 0, h2s: float = 0, tc: float = 0, pc: float = 0)-> np.ndarray:
...
def gas_ugz(p: npt.ArrayLike, sg: float, degf: float, zee: npt.ArrayLike)-> np.ndarray:
...
def gas_ponz2p(poverz: npt.ArrayLike, sg: float, degf: float, zmethod: z_method=z_method.DAK, cmethod: c_method=c_method.PMC, n2: float = 0, co2: float = 0, h2s: float = 0, tc: float = 0, pc: float = 0, rtol: float = 1E-7)-> np.ndarray:
    ...
    def PonZ2P_err(args, p):
        ...
        zee = gas_z(p=p, degf=t, sg=sg, tc=tc, pc=pc, n2=n2, co2=co2, h2s=h2s, zmethod=zmethod, cmethod=cmethod)
        return (p - (ponz * zee))/p
...
def gas_grad2sg(grad: float, p: float, degf: float, zmethod: z_method=z_method.DAK, cmethod: c_method=c_method.PMC, n2: float = 0, co2: float = 0, h2s: float = 0, tc: float = 0, pc: float = 0, rtol: float = 1E-7)-> float:
...
def gas_dmp(p1: float, p2: float, degf: float, sg: float, zmethod: z_method=z_method.DAK, cmethod: c_method=c_method.PMC, n2: float = 0, co2: float = 0, h2s: float = 0, tc: float = 0, pc: float = 0)-> float:
    ...
    def m_p(p, *args):
        ...
        mugz = gas_ugz(p, degf, sg, zee) # Gas viscosity z-factor product using a precalculated Z factor
        return  2 * p / (mugz) 
    if p1 == p2:
        return 0 
    zmethod, cmethod = validate_methods(['zmethod', 'cmethod'],[zmethod, cmethod])
...
def oil_twu_props(mw: float, ja: float = 0, sg: float = 0, damp: float = 1) -> tuple:
...
def sg_evolved_gas(p: float, degf: float, rsb: float, api: float, sg_sp: float) -> float:
...
def oil_pbub(api: float, degf: float, rsb: float, sg_g: float=0, sg_sp: float=0, pbmethod: pb_method=pb_method.VALMC) -> float:
...
def oil_rs_bub(api: float, degf: float, pb: float, sg_g: float=0, sg_sp: float=0, pbmethod: pb_method=pb_method.VALMC, rsmethod: rs_method=rs_method.VELAR) -> float:
    ...
    pbmethod, rsmethod = validate_methods(['pbmethod', 'rsmethod'],[pbmethod, rsmethod])
    ...
    def rsbub_valko_mccain(api, degf, pb, sg_g, sg_sp) -> float:
        ...
        while abs(new_err) > 1e-5:
            ...
            if new_rsb > 10:
                ...
                new_pbcalc = oil_pbub(degf = degf, api = api, sg_sp = sg_sp, rsb=new_rsb, pbmethod='STAN')
...
def oil_rs(api: float, degf: float, sg_sp: float, p: float, pb: float=0, rsb: float=0, rsmethod: rs_method=rs_method.VELAR, pbmethod: pb_method=pb_method.VALMC) -> float:
    ...
    if pb <=0: # Calculate Pb
            pb = oil_pbub(api=api, degf=degf, rsb=rsb, sg_sp=sg_sp, pbmethod=pbmethod)
    if rsb <=0: # Calculate rsb
        rsb = oil_rs_bub(api=api, degf=degf, pb=pb, sg_sp =sg_sp, pbmethod =pbmethod, rsmethod=rsmethod)
    
    if p > pb:
        return rsb
    
    def Rs_velarde(api, degf, sg_sp, p, pb, rsb):  # Velarde, Blasingame & McCain (1997)  
    ...
    return fn_dic[rsmethod.name](api=api, degf=degf, sg_sp=sg_sp, p=p, pb=pb, rsb=rsb)

def check_sgs(sg_g: float, sg_sp: float, rst: float=5 , rsp: float = 1000, sg_st: float = 1.15) -> tuple:
...
def oil_co(p: float, api: float,  degf: float, sg_sp: float=0, sg_g: float=0, pb: float=0, rsb: float=0, pi: float=0, comethod: co_method=co_method.EXPLT, zmethod: z_method=z_method.DAK, rsmethod: rs_method=rs_method.VELAR, cmethod: c_method=c_method.PMC, denomethod: deno_method=deno_method.SWMH, bomethod: bo_method=bo_method.MCAIN, pbmethod: pb_method=pb_method.VALMC):
    ...
    zmethod, rsmethod, cmethod, denomethod, bomethod, pbmethod, comethod = validate_methods(['zmethod', 'rsmethod', 'cmethod', 'denomethod', 'bomethod', 'pbmethod', 'comethod'],[zmethod, rsmethod, cmethod, denomethod, bomethod, pbmethod, comethod])
    ...
    def Co_explicit(p=p, api=api, sg_sp=sg_sp, sg_g=sg_g, degf=degf, pb=pb, rsb=rsb, zmethod=zmethod, rsmethod=rsmethod, cmethod=cmethod, denomethod=denomethod, bomethod=bomethod): # Explicit - Calculate with numerical derivatives
        ...
        bo = calc_dbodp(p)
        bg = gas_bg(p=p, sg=sg_g, degf=degf, zmethod=zmethod, cmethod=cmethod)/5.61458 # rb/scf
        return -1/bo*(dbodp - bg*drsdp)

    fn_dic = {'EXPLT': Co_explicit}
    
    return fn_dic[comethod.name](p=p, api=api, sg_sp=sg_sp, sg_g=sg_g, degf=degf, pb=pb, rsb=rsb, zmethod=zmethod, rsmethod=rsmethod, cmethod=cmethod, denomethod=denomethod, bomethod=bomethod)

def oil_deno(p: float, degf:float, rs:float, rsb:float, sg_g: float = 0, sg_sp: float = 0, pb: float = 1e6, sg_o:float=0, api:float=0, denomethod: deno_method=deno_method.SWMH) -> float:
...
def oil_bo(p:float, pb: float, degf: float, rs:float , rsb:float,sg_o:float, sg_g:float=0, sg_sp: float=0,  bomethod: bo_method=bo_method.MCAIN, denomethod: deno_method=deno_method.SWMH) -> float:
...
def make_bot_og(pi: float, api: float, degf: float, sg_g: float, pmax: float, pb: float=0, rsb: float=0, pmin: float=25, nrows: int = 20, wt: float=0, ch4_sat: float=0, comethod: co_method=co_method.EXPLT, zmethod: z_method=z_method.DAK, rsmethod: rs_method=rs_method.VELAR, cmethod: c_method=c_method.PMC, denomethod: deno_method=deno_method.SWMH, bomethod: bo_method=bo_method.MCAIN, pbmethod: pb_method=pb_method.VALMC, export: bool=False, pvto: bool=False) -> dict:
    """
    ...
    pb_i = pb
    rsb_frac = 1 # This is the fractional increase in rsb that Pb needs for solution.
    ...
    if pb_i > 0 and rsb_i > 0: 
        if not pvto: # No need to solve this if just resolving later
            print('Iteratively solving for Rsb fraction to use in order to harmonize user specified Pb and Rsb\n')
            pbcalc = oil_pbub(degf = degf, api = api, sg_sp = sg_g, rsb=rsb, pbmethod=pbmethod)
            err = 100
            rsb_old = rsb
            i = 0
            while err > 0.0001:
                rsbnew = pb/pbcalc * rsb_old
                pbcalc = oil_pbub(degf = degf, api = api, sg_sp = sg_g, rsb=rsbnew, pbmethod=pbmethod)
                rsb_old = rsbnew
                err = abs(pb - pbcalc)
                i += 1
                if i > 100:
                    ...
                    sys.exit()
            rsb_frac = rsb_i/rsbnew # Ratio of rsb needed to satisfy rsb defined by user at pb vs that needed to calculate Pb
    rsb_max = rsb     
               
    if pvto and pmax > pb:  # PVTO has been requested, and Pb is less than max pressure requested
        # Need to find new rsb_frac that delivers rsb_i at pb_i (after depletion from pmax)
        print('Iteratively solving for Rsb fraction to use at maximum pressure to deliver appropriate Pb and Rsb\n')
        rsb_max = oil_rs_bub(degf = degf, api = api, sg_sp = sg_g, pb = pmax, pbmethod=pbmethod, rsmethod=rsmethod) 
        rs_at_pbi = oil_rs(api=api, degf=degf, sg_sp = sg_g, p=pb, pb=pmax, rsb=rsb_max, rsmethod=rsmethod, pbmethod=pbmethod)
        err = rs_at_pbi - rsb
        rsb_old = rs_at_pbi
        ...
        while abs(err) > 0.0001:
            ...
            rs_at_pbi = oil_rs(api=api, degf=degf, sg_sp = sg_g, p=pb, pb=pmax, rsb=rsb_max * rsb_frac_new, rsmethod=rsmethod, pbmethod=pbmethod)
    ...
    co, cg, rss, bos, uos, gfvf, visg, gz, rvs, sg_rs, bws, visws, denos = [[] for x in range(13)]
    ...
    for p in pressures:
        if p > pb:
            ...
            rss.append(oil_rs(api=api, degf=degf, sg_sp = sg_g, p=p, pb=pb, rsb=rsb/rsb_frac, rsmethod=rsmethod, pbmethod=pbmethod)*rsb_frac)
                    
        bos.append(oil_bo(p=p, pb=pb, degf=degf, rs=rss[-1], rsb = rsb, sg_g=sg_g, sg_sp=sg_g, sg_o=sg_o, denomethod=denomethod, bomethod=bomethod))
        denos.append(oil_deno(p=p, degf=degf, rs=rss[-1], rsb = rsb, sg_g=sg_g, sg_sp=sg_sp, pb=pb, sg_o=sg_o, api=api))
        uos.append(oil_viso(p=p, api=api, degf=degf, pb=pb, rs=rss[-1]))
        co.append(oil_co(p=p, api=api, sg_sp=sg_sp, sg_g=sg_sp, degf=degf, pb=pb, rsb=rss[-1], zmethod=zmethod, rsmethod=rsmethod, cmethod=cmethod, denomethod=denomethod, bomethod=bomethod))
 
        gfvf.append(gas_bg(p=p, sg=sg_sp, degf=degf, zmethod=zmethod, cmethod=cmethod)*1000/5.61458) # rb/mscf
        gz.append(gas_z(p=p, sg=sg_sp, degf=degf, zmethod=zmethod, cmethod=cmethod)) 
        visg.append(gas_ug(p=p, sg=sg_sp, degf=degf, zmethod=zmethod, cmethod=cmethod))
    ...
    if pvto:
        ...
        for i, p in enumerate(pressures):
            ...
            try:
                ...
                usat_bo.append([oil_bo(p=pusat, pb=p, degf=degf, rs=rss[i], rsb = rss[i], sg_g=sg_g, sg_sp=sg_g, sg_o=sg_o, denomethod=denomethod, bomethod=bomethod) for pusat in usat_p[-1]])
                usat_uo.append([oil_viso(p=pusat, api=api, degf=degf, pb=p, rs=rss[i]) for pusat in usat_p[-1]])
            except:
                pass
    ...
    if export:
        ...
        if pvto: # Also export PVTO include file;
            pvto_out = 'PVTO\n'
            headers=['-- Rs (mscf/stb)', 'P (psia)', 'Bo (rb/stb)', 'uo (cP)', '']
            table = []
            for r, row in df.iterrows():
                table.append([row['Rs (mscf/stb)'], row['Pressure (psia)'], row['Bo (rb/stb)'], row['uo (cP)'],'/'])
                try:
                    if r > 0:
                        for e, entry in enumerate(usat_p[r-1]):
                            if e == 0:
                                continue
                            table.append([' ', entry, usat_bo[r-1][e], usat_uo[r-1][e],' '])
                except:
                    pass
...
def lorenz_from_flow_fraction(kh_frac: float, phih_frac: float, lrnz_method: str = 'EXP') -> float:
...
def lorenz_2_flow_frac(lorenz: float, phih_frac: float, lrnz_method: str = 'EXP', B: float = -1) -> float:
...
def lorenz_2_layers(lorenz: float, k_avg: float, nlayers: int=1 , shuffle: bool = False, lrnz_method: str = 'EXP', B: float = -1, phi_h_fracs: list = []) -> np.ndarray:
    ...
    kh = np.array([sumkh[i]-sumkh[i-1] for i in range(1,len(sumkh))])*k_avg
...
def ix_extract_problem_cells(filename: str='', silent: bool=False) -> list:
    ...
    while True:
        ...
        if 'INTERSECT is a mark of Chevron Corporation, Total S.A. and Schlumberger' in line:
            ix_found = True
        if not line:
            break
...
def rel_perm(rows: int, krtable: kr_table=kr_table.SWOF, krfamily: kr_family=kr_family.COR, kromax: float=1, krgmax: float=1, krwmax: float=1, swc: float=0, swcr: float=0, sorg: float=0, sorw: float=0, sgcr: float=0, no: float=1, nw: float=1, ng: float=1, Lw: float=1, Ew: float=1, Tw: float=1, Lo: float=1, Eo: float=1, To: float=1, Lg: float=1, Eg: float=1, Tg: float=1, export: bool=False)-> pd.DataFrame:
    ...
    def kr_SWOF(rows: int, krtable: kr_table=kr_table.SWOF, krfamily: kr_family=kr_family.COR, kromax: float=1, krgmax: float=1, krwmax: float=1, swc: float=0, swcr: float=0, sorg: float=0, sorw: float=0, sgcr: float=0, no: float=1, nw: float=1, ng: float=1, Lw: float=1, Ew: float=1, Tw: float=1, Lo: float=1, Eo: float=1, To: float=1, Lg: float=1, Eg: float=1, Tg: float=1)-> pd.DataFrame:
    ...
    def kr_SGOF(rows: int, krtable: kr_table=kr_table.SWOF, krfamily: kr_family=kr_family.COR, kromax: float=1, krgmax: float=1, krwmax: float=1, swc: float=0, swcr: float=0, sorg: float=0, sorw: float=0, sgcr: float=0, no: float=1, nw: float=1, ng: float=1, Lw: float=1, Ew: float=1, Tw: float=1, Lo: float=1, Eo: float=1, To: float=1, Lg: float=1, Eg: float=1, Tg: float=1)-> pd.DataFrame:
    ...
    def kr_SGWFN(rows: int, krtable: kr_table=kr_table.SWOF, krfamily: kr_family=kr_family.COR, kromax: float=1, krgmax: float=1, krwmax: float=1, swc: float=0, swcr: float=0, sorg: float=0, sorw: float=0, sgcr: float=0, no: float=1, nw: float=1, ng: float=1, Lw: float=1, Ew: float=1, Tw: float=1, Lo: float=1, Eo: float=1, To: float=1, Lg: float=1, Eg: float=1, Tg: float=1)-> pd.DataFrame:
    ...
    if krtable.name == 'SWOF':
        return kr_SWOF(rows, krtable, krfamily, kromax, krgmax, krwmax, swc, swcr, sorg, sorw, sgcr, no, nw, ng, Lw, Ew, Tw, Lo, Eo, To, Lg, Eg, Tg)
    if krtable.name == 'SGOF':
        return kr_SGOF(rows, krtable, krfamily, kromax, krgmax, krwmax, swc, swcr, sorg, sorw, sgcr, no, nw, ng, Lw, Ew, Tw, Lo, Eo, To, Lg, Eg, Tg)
    if krtable.name == 'SGWFN':
        return kr_SGWFN(rows, krtable, krfamily, kromax, krgmax, krwmax, swc, swcr, sorg, sorw, sgcr, no, nw, ng, Lw, Ew, Tw, Lo, Eo, To, Lg, Eg, Tg)
    print( 'Check that you have specified table type as SWOF, SGOF or SGWFN')
    sys.exit() 
    
def influence_tables(ReDs: list, min_td: float=0.01, max_td: float=200, n_incr: int=20, M: int=8, export: bool=False)-> tuple:
    """ Returns a tuple of;
    ...
    def laplace_Qs(s: float, ReD: float):
        ...
        denominator = s**1.5*(k1sReD*mp.besseli(0,x)+i1sReD*mp.besselk(0, x))
    ...
    def laplace_Ps(s: float, ReD: float):
        ...
        denominator = s**1.5*(i1sReD*mp.besselk(1, x)-k1sReD*mp.besseli(1,x))
        return numerator/denominator
    ...
    if export:
        ...
        for i, ReD in enumerate(ReDs):
            ...
            inc_out += '----\n'
            inc_out += '----  re/rw = '+str(ReD)+', No flow outer boundary\n'
            header = ['--','Table #', 're/rw']
            table = [['--','tD', 'pD']]
